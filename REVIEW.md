...
1. **A golden-graph test and a complete replay key.** walk-15 on the three parity seeds and one MangoDB brief, from the recording and the model cache, asserting node count, statuses and cited titles. Then the rest is safe. *Landed 2026-09-19: the golden graph (4aa0cdd, `test/golden.test.js`), and the replay key carries the adapter's config (extra request fields, context window); every cached entry was rewritten to the new key and the four suites replayed with zero misses.*
...
3. **Run state into the run**: kept, frontier, judged and the node fields as serialisable fields; export mid-run, reimport, finish. *Landed 2026-09-19: `run.state` (graph.js `stateOf`) holds kept, opened, links and judged as plain data; `validateImport(text, { resume: true })` continues a run; `test/resume.test.js` exports the Turing walk at four steps, imports it and finishes to the uninterrupted graph. The node fields wait for the kinds in step 4.*
4. **Node kinds and one resolution path**: a `kind` with a table; `resolveWith` and the settled-children block become one `gather`; the source profile replaces the `line` flag; `subjectIgnore` moves to the run. Then drop `UNWANTED_NAME` and `CHECK_FOREIGN` if no row moves. *Landed 2026-09-19 in three stages, each replayed with zero misses and no row moved at 1.7B, 4B, 8B and Haiku: `kindOf` (question, split, brief, section, hop) set when the walk creates a node and read where a flag was; the ignore list is a walk option; `chooseArticle` is the one article choice for both search sites; `resolve` is the one way a node resolves (own kept text, then settled children, one de-dup, one cap, one fragment rule). Still to try: dropping `UNWANTED_NAME` and `CHECK_FOREIGN`.*
