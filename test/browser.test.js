...
test("the driver hooks: a live run with custom limits, and Wikipedia served from a loaded recording without the network", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    assert.deepEqual(limits, { maxNodes: 40, maxVisits: 60, maxDepth: 0, maxLookups: 6, maxPasses: 8, revisitSettled: true, walk: true, maxSentences: 3, maxFindingChars: 6000, maxSections: 6 });
