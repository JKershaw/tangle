...
test("every ask variant builds calls with a tiny schema and reads the answer back", () => {
  const input = { question: "Why?", sentences: ["Because A.", "Because B."], article: "X", sections: ["One", "Two"], titles: ["X", "Y"], snippets: ["x is", "y is"], sentence: "Because A.", actions: [{ name: "npm run test", description: "run the tests" }] };
