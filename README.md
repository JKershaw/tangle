...
- The fair control exists and has been run: the same model with the four source requests as tools in one context, each seed budgeted to the graph's own calls and tokens. At 1.7B and 4B the graph beats it about two to one on every brief set, and the tool control equals memory. At 8B the tool control closes to within a few topics on Wikipedia and beats the graph on MangoDB at a third of the tokens per topic; at 14B the graph leads again on both, at three to four times the tokens per topic. Over an API the same holds: Haiku 4.5 through the graph reads MangoDB better than Haiku driving itself (25 against 20 of 42, the best code row, all cited) and DeepSeek V3.2 turns out to be a poor picker at any price. Asked to answer only in sentences it read, a single context paraphrases instead at every size below 8B; the graph cites because a finding is a pick.

**In progress.** Roadmap milestone 5: a directory of files as a second source, with [MangoDB](https://github.com/JKershaw/mangodb) (19,000 lines of TypeScript and a four-second test suite) as the first corpus. A file is an article, its declarations are sections, its lines are sentences, and the declarations a line uses are its links. It runs in Node (`--source <dir>`); the page does not read files yet. First rows: the graph beats one node and memory at 1.7B, 4B and 8B (14 / 11 / 12 of 42 topics against 7 / 2 / 3 and 9 / 8 / 9), and memory invents a write-ahead log the database does not have. It reads most of the rubric and keeps a third to a half: the sentence pick over lines of code is the frontier. Milestones 5a (the replay), 5b (the tool control) and 5c (the foundations review, [REVIEW.md](REVIEW.md), landed the same day) are exited. Milestone 6 has begun: a node whose finding is the result of a command run in a worktree of the corpus, picked by the model from a list code prepared; at 1.7B and 4B the graph ran MangoDB's 1,730 tests and cited the runner's summary. Rows in PROGRESS.md; the reading in evals/readings.md. The milestones and their exit conditions are in [ROADMAP.md](ROADMAP.md).
