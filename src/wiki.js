...
// The subject's naming words (text.js: capitalised, with their capitals),
// else the phrase as it is.
export function aboutWords(phrase) {
  const { cased } = subjectWords(phrase);
  return cased.length ? cased : String(phrase ?? "").split(/\s+/).filter(Boolean);
}
