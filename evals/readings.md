...
**What the profiles are.** Seven or eight paragraphs, every line cited to a file and line range, most of them JSDoc summaries and inline comments, with the signature they describe; the model prefers a comment to the code below it, which for a reader is right and for a rubric of identifiers is not. A JSDoc `@example` line is kept as readily as a `@description`, and an example is not a finding. Sections are chosen by name alone, and 1.7B chooses `bulkWrite` and `insertOne` for persistence where `writeDocuments` and `readDocuments` are the answer; at 8B the picks are better. The three faults still open, each a case for a node eval: examples as sentences, sections by name alone, and a declaration of one line offered as a section.

**Cost.** A 1.7B profile over MangoDB is seven to nine nodes, thirty calls, ten thousand tokens and five to seven seconds in Node. That is the number to set beside the warm-graph row when it exists.
