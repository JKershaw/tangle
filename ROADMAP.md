...
- **Exit:** we know whether the graph beats a tool-using single context at equal budget, and at which size the answer changes. Actions are not built until this says the graph earns its keep. **Exited 2026-09-19 (a811bbd):** at 1.7B and 4B the graph beats the tool control on every brief set about two to one (profiles 25 / 24 against 13 / 11, overflow 24 / 23 against 11 / 11, MangoDB 14 / 11 against 6 / 11) and the tool control equals memory; at 8B the tool control closes to within three or four topics on Wikipedia and beats the graph on MangoDB (15 against 12) at a third of the tokens per topic, and per token an 8B call costs about five 1.7B ones. The cited variant is near zero below 8B. So the graph earns its keep where the thesis lives, small models, and not at 8B on code. The warm-graph row is not built: no node question repeats across the four MangoDB briefs at any size, so there is nothing for it to reuse yet. Table in PROGRESS.md; reading in [evals/readings.md](evals/readings.md). DeepSeek over the key is still unrun.

### 6. Actions and blocking

- **Entry:** milestone 5b, and only if it says the graph earns its keep. It does at 1.7B and 4B on briefs and not at 8B on code (2026-09-19); whether to build actions for the small sizes is the next decision.
