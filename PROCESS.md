...
2. **The replay loop, seconds.** A rerun of a suite with the model's responses cached by their exact context (roadmap milestone 5a). Unchanged prompts replay; changed ones go to the model. The miss count is the blast radius of the change, and a row that moved with zero misses is a bug in code. Until the cache exists, this loop is the eval loop and costs minutes; do not use it to find bugs in code.
